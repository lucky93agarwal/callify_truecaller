...
class CallingActivity : AppCompatActivity(), ServiceConnection {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d("UpdateUIUpdatephoneNumber","Check Calling Type ==       "+intent.getIntExtra(IntentUtils.CALL_TYPE,  CallType.VOICE.value))
        Log.d("UpdateUIUpdatephoneNumber","Check Calling phoneNumber ==       "+phoneNumber)
        Log.d("UpdateUIUpdatephoneNumber","Check Calling callType ==       "+callType)

        Log.d("UpdateUIUpdatephoneNumber","Check Calling uid ==       "+uid)

        callDirection = intent.getIntExtra(IntentUtils.CALL_DIRECTION, -1)
        mCallId = intent.getStringExtra(IntentUtils.CALL_ID)
        Log.d("UpdateUIUpdatephoneNumber","Check Calling mCallId ==       "+mCallId)
        ...
        if (user != null) {
            user?.let { user ->
                ...
                Log.d("UpdateUIUpdateCalling","calling State 3    ====     "+R.string.connecting);
...
//                            }).addTo(disposables)
        ...
        constraint.setOnClickListener(View.OnClickListener {
            ...
            if (rtcEngine()?.connectionState != Constants.CONNECTION_STATE_CONNECTED) return@OnClickListener
...
//            ) { throwable: Throwable? -> }.addTo(disposables)
        ...
        Log.d("UpdateUIUpdate","Check Camera know  1    =    "+isVideoCall());
        updateUI()
...
//        Log.d("UpdateUIUpdateCalling","calling State n786    ====     "+callingState);
        ...
        when (callingState) {
            ...
            CallingState.ERROR -> {
                Log.d("UpdateUIUpdateCalling","calling State n786    ====     "+"Error");
            }
            else -> {
                tvStatus.text = ""
            }
    ...
    private fun hideOrShowTopBottomHolders(setHidden: Boolean) {
        ...
        constraint1.clone(this, R.layout.activity_calling)
    ...
    fun updateState(callingViewState: CallingViewState) {
        ...
        when (callingViewState) {
            ...
            is CallingViewState.UpdateDuration -> {
                Log.d("UpdateUIUpdateCalling","calling State 2    ====     "+Util.formatCallTime(callingViewState.duration.toInt()));
            ...
            is CallingViewState.MicMuted -> {
                Log.d("UpdateUIUpdateCalling","calling State Muted 2    ====     "+callingViewState.setMuted);
                val isMicMuted = callingViewState.setMuted
                setIconBg(btnMic, isMicMuted)
            }

            is CallingViewState.SetupRemoteViewForUid -> {
                Log.d("UpdateUIUpdateCalling","calling State id 2    ====     "+callingViewState.uid);
            ...
            is CallingViewState.UpdateCallingState -> {
                Log.d("UpdateUIUpdateCalling","callingViewState.callingState    ====     "+callingViewState.callingState);
            ...
            is CallingViewState.JoinChannelSuccess -> {
                volumeControlStream = AudioManager.STREAM_VOICE_CALL
                hideOrShowButtons(true)
                Log.d("UpdateUIUpdateCalling","callingViewState  JoinChannelSuccess    ====     "+volumeControlStream);
                if (isVideoCall())
                    setVideoStuff()
                updateUI()
            }
            ...
            is CallingViewState.HideRemoteViews -> {
                Log.d("UpdateUIremote_viewUpdateCalling","======   1    ====     ");
    ...
    private fun resumeLocalVideo() {
        ...
        Log.d("UpdateUIUpdate","Check Camera know  2    =    ");
...
//which will cause a Flicker
    ...
    private fun addLocalView() {
        Log.d("UpdateUIUpdatephoneNumber","Check Calling baseContext ==       "+baseContext)
        ...
        Log.d("UpdateUIUpdate","Check Camera know  4    =    ");
    ...
    private fun removeLocalView() {
        Log.d("UpdateUIUpdate","Check Camera know  3    =    ");
    ...
    private fun removeRemoteView(uid: Int) {
        ...
        Log.d("UpdateUIremote_viewUpdateCalling","======   2    ====     ");
    ...
    private fun muteOrUnMuteRemoteView(uid: Int, setMuted: Boolean) {
        Log.d("UpdateUIremote_viewUpdateCalling","======   7    ====     ");
    ...
    private fun updateUI() {
        ...
        if (isVideoCall()) {
            ...
            Log.d("UpdateUIUpdate","Check data 1");
            ...
            Log.d("UpdateUIUpdate","Check data 2");
...
//also it will prevent the screen from auto turn off when the user the is making a call
    ...
    override fun onServiceConnected(componentName: ComponentName, iBinder: IBinder) {
        Log.d("UpdateUIUpdateCalling","callingViewState  onServiceConnected    ====     ");
        if (CallingService::class.java.name == componentName.className) {
            ...
            when (action) {
                IntentUtils.ACTION_START_NEW_CALL -> {
                    ...
                    val fireCall = FireCall(image_name, user, FireCallDirection.OUTGOING, System.currentTimeMillis(), phoneNumber
                            ?: "", isVideoCall(), callType.value, channel)
                    Log.d("UpdateUIUpdateCalling","callingViewState  onServiceConnected    ====  fireCall   ===========   "+fireCall);
    ...
    override fun onServiceDisconnected(componentName: ComponentName) {
        Log.d("UpdateUIUpdateCalling","callingViewState  onServiceDisconnected    ====     ");
        if (CallingService::class.java.name == componentName.className) {
    ...
    private fun renderRemoteVideo(uid: Int) {
        Log.d("UpdateUIUpdatephoneNumber","renderRemoteVideo uid uid ==       "+uid)
    ...
    private fun addRemoteView(uid: Int, surfaceV: SurfaceView) {
        Log.d("UpdateUIremote_viewUpdateCalling","======   4    ====     ");
