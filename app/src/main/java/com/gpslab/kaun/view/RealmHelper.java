...
public class RealmHelper {
    ...
    public void saveObjectToRealm(RealmObject object) {
        Log.d("CheckChatAPI","object 1 = "+String.valueOf(object));
    ...
    public void saveLastMessageForChat(String chatId, Message message) {
        ...
        Log.d("migratelucky","saveLastMessageForChat   ==   1 ");
        realm.beginTransaction();
    ...
    public void saveLastMessageForChat(Message message) {
        ...
        if (chat == null)
            return;
        Log.d("migratelucky","saveLastMessageForChat   ==   2 ");
...
//        if (!isChatStored(chatId)) {
            Log.d("CheckDataAdapter","User chatId = 2 "+chatId);
            Chat chat = new Chat();
...
*/
    ...
    public void updateDownloadUploadStat(String messageId, int downloadUploadStat) {
        ...
        Log.d("updateDowloadUpload","message id 7 =   "+messageId+"    downloadUploadStat 7   =   "+downloadUploadStat);
        RealmResults<Message> messages = getMessages(messageId);

        Log.d("updateDowloadUpload","message id 7 =   "+messages);
        realm.beginTransaction();
        for (Message message : messages) {
            Log.d("updateDowloadUpload","get message id 11 =   "+message.getMessageId());
            ...
            if (downloadUploadStat == DownloadUploadStat.SUCCESS){
                ...
                Log.d("updateDowloadUpload","get message id 22  =   "+message.getMessageId());
            }
    ...
    //update upload state when it's finished whether it's success ,failed,loading or cancelled
    public void updateDownloadReadStat(String messageId, int downloadUploadStat) {
        //we are getting all messages because it's may be a broadcast ,if so we want to update the state of all of them
        Log.d("updateDowloadUpload","message id 7 =   "+messageId+"    downloadUploadStat 7   =   "+downloadUploadStat);
        RealmResults<Message> messages = getMessages(messageId);
        ...
        for (Message message : messages) {
            ...
            //if upload state is success ,update the message state to sent
            if (downloadUploadStat == DownloadUploadStat.SUCCESS){
                message.setMessageStat(MessageStat.READ);
                Log.d("updateDowloadUpload","get message id 22  =   "+message.getMessageId());
            }
    ...
    public void updateDownloadSendStat(String messageId, int downloadUploadStat) {
        //we are getting all messages because it's may be a broadcast ,if so we want to update the state of all of them
        Log.d("msg_read_report_sender","message id 7 =   "+messageId+"    downloadUploadStat 7   =   "+downloadUploadStat);
        RealmResults<Message> messages = getMessages(messageId);

        Log.d("msg_read_report_sender","message id 7 =   "+messages);
        realm.beginTransaction();
        for (Message message : messages) {
            Log.d("msg_read_report_sender","get message id 11 =   "+message.getMessageId());
            //if upload state is success ,update the message state to sent
            if (downloadUploadStat == DownloadUploadStat.SUCCESS){
                message.setMessageStat(MessageStat.RECEIVED);
                Log.d("msg_read_report_sender","get message id 22  =   "+message.getMessageId());
            }
    ...
    public void updateDownloadSend(String messageId, int downloadUploadStat) {
        //we are getting all messages because it's may be a broadcast ,if so we want to update the state of all of them
        Log.d("updateDowloadUpload","message id 7 =   "+messageId+"    downloadUploadStat 7   =   "+downloadUploadStat);
        RealmResults<Message> messages = getMessages(messageId);
    ...
    public void updateDownloadSendREAD(String messageId, int downloadUploadStat) {
        //we are getting all messages because it's may be a broadcast ,if so we want to update the state of all of them
        Log.d("updateDowloadUpload","message id 7 =   "+messageId+"    downloadUploadStat 7   =   "+downloadUploadStat);
        RealmResults<Message> messages = getMessages(messageId);
...
//    }
    ...
    public void migrateChat(Chat mChat) {
        ...
        realm.beginTransaction();
        Log.d("migratelucky","migrateChat   ==    ");
