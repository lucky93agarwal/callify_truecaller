...
public class CropFragment extends BaseFragment implements View.OnClickListener{
    ...
    public void setImageBitmap(Bitmap bitmap) {
        ieeCropImageView.setImageBitmap(bitmap);
    }
    ...
    @Override protected void initView(View view) {
        ieeCropImageView = view.findViewById(R.id.image_iv);
        ...
        if(getArguments()!=null) {
            ...
            if(bitmapimage!=null){
                ieeCropImageView.setImageBitmap(bitmapimage);
//                ieeCropImageView.setAspectRatio(1, 1);
//                ieeCropImageView.setGuidelines(IEECropImageView.Guidelines.ON_TOUCH);
                final Rect cropRect = getArguments().getParcelable(ImageEditor.EXTRA_CROP_RECT);
                if(cropRect!=null){
                    ieeCropImageView.setCropRect(cropRect);
                }
            }
        }
    }
    ...
    @Override public void onClick(View view) {
        if(view.getId()==R.id.rotate_iv){
            ieeCropImageView.rotateImage(90);
        }
            ...
            mListener.onImageCropped(ieeCropImageView.getCroppedImage(), ieeCropImageView.getCropRect());
        }
        else if(view.getId()==R.id.done_tv){
