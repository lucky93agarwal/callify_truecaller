...
class IEECropOverlayView extends View {
    ...
    private IEECropImageView.Guidelines mGuidelines;

    /** The shape of the cropping area - rectangle/circular. */
    private IEECropImageView.CropShape mCropShape;
    ...
    public IEECropOverlayView(Context context) {
        this(context, null);
    }

    public IEECropOverlayView(Context context, AttributeSet attrs) {
        super(context, attrs);
    }
    ...
    /** The shape of the cropping area - rectangle/circular. */
    public IEECropImageView.CropShape getCropShape() {
        return mCropShape;
    }

    /** The shape of the cropping area - rectangle/circular. */
    public void setCropShape(IEECropImageView.CropShape cropShape) {
        if (mCropShape != cropShape) {
            mCropShape = cropShape;
            if (Build.VERSION.SDK_INT <= 17) {
                if (mCropShape == IEECropImageView.CropShape.OVAL) {
    ...
    public IEECropImageView.Guidelines getGuidelines() {
        return mGuidelines;
    }
     ...
     * application.
     */
    public void setGuidelines(IEECropImageView.Guidelines guidelines) {
    ...
    public boolean setMultiTouchEnabled(boolean multiTouchEnabled) {
        if (mMultiTouchEnabled != multiTouchEnabled) {
            ...
            if (mMultiTouchEnabled && mScaleDetector == null) {
                mScaleDetector = new android.view.ScaleGestureDetector(getContext(), new ScaleListener());
            }
            return true;
        }
        return false;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mCropWindowHandler.showGuidelines()) {
            ...
            if (mGuidelines == IEECropImageView.Guidelines.ON) {
                drawGuidelines(canvas);
            } else if (mGuidelines == IEECropImageView.Guidelines.ON_TOUCH && mMoveHandler != null) {
    ...
    private void drawBackground(Canvas canvas) {
        ...
        if (mCropShape == IEECropImageView.CropShape.RECTANGLE) {
            ...
            mPath.reset();
            if (Build.VERSION.SDK_INT <= 17 && mCropShape == IEECropImageView.CropShape.OVAL) {
    ...
    private void drawCorners(Canvas canvas) {
        if (mBorderCornerPaint != null) {
            ...
            float w =
                    cornerWidth / 2
                            + (mCropShape == IEECropImageView.CropShape.RECTANGLE ? mBorderCornerOffset : 0);
