package com.gpslab.kaun.Forward;
...
import com.gpslab.kaun.status.BaseActivity;
    ...
    private void handleIncomingAudio() {
        ...
        if (adapter.getSelectedForwardedUsers().size() > 1) {
            for (User user : adapter.getSelectedForwardedUsers()) {
                ...
                Message message = new MessageCreator.Builder(user, MessageType.SENT_AUDIO,image_name).path(filePath).duration(length).build();
           //     ServiceHelper.startNetworkRequest(this, message.getMessageId(), message.getChatId());
            ...
            edit.putString("chat_id",user.getUid());
            edit.apply();
    ...
    private void handleVideoShare() {
        ...
        if (adapter.getSelectedForwardedUsers().size() > 1) {
            ...
            for (User user : adapter.getSelectedForwardedUsers()) {
                ...
                RandomString randomString = new RandomString();
                String image_name = ts + "_" + randomString.nextString();
                Message message = new MessageCreator.Builder(user, MessageType.SENT_VIDEO,image_name).context(this).path(filePath).build();
              //  ServiceHelper.startNetworkRequest(this, message.getMessageId(), message.getChatId());
                showSendingToast();
                finish();
            }
                ...
                for (User user : selectedUsers) {
                    ...
                    for (Uri uri : imageUris) {
                        ...
                        if (filePath != null) {
                            ...
                            RandomString randomString = new RandomString();
                            String image_name = ts + "_" + randomString.nextString();
                            Message message = new MessageCreator.Builder(user, MessageType.SENT_IMAGE,image_name).path(filePath).fromCamera(false).build();
//                            ServiceHelper.startNetworkRequest(this, message.getMessageId(), message.getChatId());
                        } else {
                            showFileNotFoundToast();
                        }
...
//                    ServiceHelper.startNetworkRequest(this, message.getMessageId(), message.getChatId());
    ...
    private void startTheActivityWithFlags(Intent intent) {
        ...
        sBuilder.addNextIntentWithParentStack(new Intent(this, MainHomeActivity.class));
    ...
    private void handleTextShare(List<User> selectedUsers) {
        ...
        if (selectedUsers.size() > 1) {
            for (User selectedUser : selectedUsers) {
                ...
                RandomString randomString = new RandomString();
                String image_name = ts + "_" + randomString.nextString();
                Message message = new MessageCreator.Builder(selectedUser, MessageType.SENT_TEXT,image_name).text(sharedText).build();
//                ServiceHelper.startNetworkRequest(this, message.getMessageId(), message.getChatId());
            }
            showSendingToast();
            finish();
    ...
    private void handleContacts(List<ExpandableContact> selectedContacts) {
        if (adapter.getSelectedForwardedUsers().size() > 1) {
            ...
            for (User user : adapter.getSelectedForwardedUsers()) {
                ...
                RandomString randomString = new RandomString();
                String image_name = ts + "_" + randomString.nextString();
                List<Message> messages = new MessageCreator.Builder(user, MessageType.SENT_CONTACT,image_name).contacts(selectedContacts).buildContacts();
